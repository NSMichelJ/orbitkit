...
export const add = new Command()
  ...
  .action(async (components: string[]) => {
    ...
    if (!components.length) {
      const availableComponents = getAllComponents().map(
      ...
      componentsToInstall.push(...(selectedComponents as string[]));
    }
    ...
    for (const componentToInstall of componentsToInstall) {
      const componentPath = getComponentPath(componentToInstall);
      ...
      spinner.text = `Adding ${componentToInstall}...`;
      try {
